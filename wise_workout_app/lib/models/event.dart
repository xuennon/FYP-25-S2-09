...
class Event {
  ...
  String get primaryMetric {
    ...
    // Business user defined metrics take priority (converted from array to map in fromMap)
    List<String> availableMetrics = metrics!.keys.toList();
    print('🎯 Event $id: Available metrics: $availableMetrics');
    
    String selectedMetric = 'durationSeconds'; // Default
    
    // Priority order for ranking metrics
    if (availableMetrics.contains('distance')) {
      selectedMetric = 'distanceKm';
    } else if (availableMetrics.contains('time')) {
      selectedMetric = 'durationSeconds';
    } else if (availableMetrics.contains('elevation')) {
      selectedMetric = 'elevation';
    } else if (availableMetrics.contains('pace')) {
      selectedMetric = 'avgPace';
    }
    
    print('🎯 Event $id: Selected primary metric: $selectedMetric');
  ...
  factory Event.fromMap(String id, Map<String, dynamic> map) {
    try {
      ...
      // Safely parse metrics - handle both array and map formats
      Map<String, dynamic>? metrics;
      if (map['metrics'] != null) {
        if (map['metrics'] is List) {
          // Array format: ["distance", "time"] -> convert to map
          List<String> metricsList = List<String>.from(map['metrics']);
          metrics = {};
          for (String metric in metricsList) {
            metrics[metric] = true; // Set as boolean flag
          }
        } else if (map['metrics'] is Map) {
          // Map format: {"distance": true, "elevation": 150}
