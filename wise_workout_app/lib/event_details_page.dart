        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              print('👥 Current event participants: ${widget.event.participants}');
              setState(() {
                _leaderboardStream = _leaderboardService.streamEventLeaderboard(widget.event.id);
              });
            },
          ),
          // Debug clean leaderboard button
          IconButton(
            icon: const Icon(Icons.cleaning_services, color: Colors.red),
            onPressed: () async {
              print('🧹 Cleaning leaderboard data for testing...');
              final currentUserId = FirebaseAuth.instance.currentUser?.uid;
              if (currentUserId != null) {
                bool success = await _leaderboardService.removeUserFromEventLeaderboard(widget.event.id, currentUserId);
                print('🗑️ Leaderboard cleanup result: $success');
                if (success) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Cleaned your leaderboard data'),
